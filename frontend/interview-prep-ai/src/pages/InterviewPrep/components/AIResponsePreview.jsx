...
const AIResponsePreview = memo(({ content }) => {
    ...
    // ✅ FIX: Memoize components untuk performance
    const components = useMemo(() => ({
        // ✅ FIX: Destructure children dan inline dari props
        code({ node, inline, className, children, ...props }) {
            const match = /language-(\w+)/.exec(className || ''); // ✅ FIX: Regex pattern
            const language = match ? match[1] : ''; 

            return !inline ? ( // ✅ FIX: Use inline instead of !className
                <CodeBlock
        ...
        blockquote({ children }) {
            return <blockquote className="border-l-4 border-gray-200 pl-4 italic my-4">
                {children}
            </blockquote>
        }, 
        h1({ children }) {
    ...
    return (
        <div className="max-w-4xl mx-auto" role="article" aria-label="AI Response Content">
...
// ✅ ADD: Display name untuk debugging
AIResponsePreview.displayName = 'AIResponsePreview';
        ...
        <div className="relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200">
            <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200">
                ...
                >
                    ...
                    {copied && (
                        <span className="absolute -top-8 right-0 bg-black text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 transition z-10">
                            Copied!
                        </span>
                    )}
                </button>
